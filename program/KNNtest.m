sl = load('C:\Users\W\Desktop\EMG data\2nddata\train_group2nd.mat');%读取collect_traindata提取出来的训练数据及标签
...
sample_group_gather = [];
for v=1:10 %v次测试
filename1=ls('C:\Users\W\Desktop\EMG data\2nddata\sample\*.xlsx');%取得所用excel文件
...
for ii=1:num
    i=randi([0 50]);
raw=xlsread(['C:\Users\W\Desktop\EMG data\2nddata\sample\',filename{ii}],1,'C50:D5500');
for j=1:segment
M=[M,raw(i:i+n,1:2)];
i= i + n + randi([0 100]);
end
...
samplecol=1;
  ...
  classifyresult = knnclassify(sample, traindata2nd, train_group2nd, 3);
       total = size(sample_group,1);%返回sample_group行数
       correct=0;
       for i=1:total
           if classifyresult(i,1)==sample_group(i,1)
               correct = correct+1 ;
           end
       end
